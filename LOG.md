...
- 당시 폴백 기록(2026-07-31 정정): 담당 에이전트가 내장 모델 목록과 리포의 API 브리지만
  확인하고 호스트의 `claude`·`agy` CLI를 탐색하지 않아 GPT 자체 검증으로 진행했다.
  CLI가 없었던 것이 아니라 **탐색 누락**이었다. 실제 호출 여부 기록 자체는 그대로 유효하다.
...
- 당시 폴백 기록(2026-07-31 정정): `claude`·`agy` CLI 탐색 누락으로 두 모델을 호출하지 않고
  GPT 검증으로 확정했다. “현재 런타임에서 호출 불가”라는 원래 판단은 사실이 아니었다.
...
## 2026-07-31 — 모델 CLI 가용성 정정

- `claude` CLI 2.1.220, `agy` CLI 1.1.8이 호스트에 설치돼 있음을 실조회했다.
- `claude -p --safe-mode --model fable --effort medium` 호출이 `claude-fable-5`로 완료되어
  `OK`를 반환했다.
- `agy models`에서 `gemini-3.6-flash-high`를 확인했고, 해당 모델의 비대화형 호출도 완료됐다.
- 따라서 앞으로 고추론 교차검증은 Claude CLI의 Fable 5 medium과 현재 Sol이 담당하고,
  저추론 수행은 AGY CLI의 Gemini 3.6 Flash high를 우선 사용한다.
- 정정 사유: Codex 런타임에 직접 노출된 모델 목록과 CLI로 호출 가능한 호스트 모델을 잘못
  동일시했다. 리포 브리지의 `.env` 부재는 CLI 가용성과 무관하다.

---

## 2026-08-04 — 사용자·프롬프트 진단 및 하네스 경량화 (세션 핸드오프)

### 사용자 요청과 접근 범위

- 사용자는 접근 가능한 프로젝트 기록과 대화 맥락을 모두 활용해 다음을 평가해 달라고 했다.
  1. 사용자의 가장 큰 장점·단점
  2. 프롬프트의 장점·단점과 처음부터 더 나았을 프롬프트 방식
  3. 프로젝트 하네스 중 불필요한 규칙의 즉시 수정
  4. 사용자에게 가장 부족하고 시급한 개선점
- 실제 근거로 이 리포의 `AGENTS.md`, `CLAUDE.md`, `LOG.md`, 채널별 HANDOFF, context 문서,
  최근 커밋과 `/home/kkokk/AX` 아래 다른 프로젝트의 AGENTS/CLAUDE 진입점을 읽었다.
- 다른 터미널의 비공개 채팅 원문은 현재 세션에서 직접 열람할 수 없으므로 봤다고 주장하지 않았다.

### 최종 진단

- 가장 큰 장점: 시행착오에서 본질을 뽑고 자기 판단도 증거에 따라 수정하는 능력. 상품 홍보를
  여행지 발견으로 재정의하고, 시리즈명·사진 우선순위·릴스 형식을 실제 독자 관점과 실물 대조로
  바꾼 기록을 근거로 삼았다.
- 가장 큰 단점: 좋은 판단을 잃지 않으려고 실패마다 규칙·예외·정정 이력을 누적해 통제 체계가
  비대해지는 경향. 그 결과 정본 중복, 오래된 상태의 잔존, 실행보다 검증·기록 관리가 커질 수 있다.
- 프롬프트 장점: 높은 기준, 명확한 금지선, 원인까지 파고드는 피드백, 증거와 자율성의 동시 요구.
- 프롬프트 단점: 실제 결과보다 수행 절차와 기존 형식을 먼저 지정하는 경우가 있어, 기존 전제를
  버려야 할 작업에서도 여러 차례 수정이 필요해진다.
- 권장 프롬프트 구조: `실제 결과 → 대상과 원하는 행동 → 절대 제약 3~5개 → 버려도 되는 기존 전제
  → 성공·중단 기준` 순서. 배경과 절차를 더 늘리기보다 성공 판정을 앞에 둔다.
- 가장 시급한 개선: 아이디어나 미감이 아니라 종료 기준과 선택적 무시 능력. 새 문서 규칙을 만들기
  전에 코드·테스트·수치로 막을 수 있는지 확인하고, 가능하면 자동 게이트로 대체한다. 동시에 중요한
  목표는 최대 3개, 목표별 대표 수치 1개, 같은 문제가 두 번 반복될 때만 새 규칙을 추가한다.

### 실제 하네스 변경

- `AGENTS.md` 시작 순서를 모델 정본 → 원칙 → 해당 채널 HANDOFF로 정리했다. `LOG.md`와 전체
  `learning/`은 필요할 때만 읽도록 바꿔 초기 컨텍스트 과적재를 줄였다.
- 장황한 Antigravity MCP 설치 예시, Claude 전용 전달 기능, Gemini/GLM 래퍼 설명을 제거하고
  실제 도구만 사용·인증 우회 금지·외부 메시지 무단 발송 금지라는 행동 규칙만 남겼다.
- `CLAUDE.md`의 과거 모델 배정과 래퍼 구현 설명 약 50줄을 제거했다. 모델 라우팅의 유일한 정본은
  `AGENTS.md` §0-A로 통일했다.
- 오래된 `HANDOFF-2026-07-29.md` 우선 읽기와 2026-07-29 대기 작업 복제본을 제거했다. 변하는
  상태는 채널별 HANDOFF에만 기록한다.
- 모든 작업에 적용하기 위험한 `기본 브랜치 직접 커밋·즉시 배포`, `병렬 에이전트 적극 활용`,
  `작업 시작 시 git pull 필수`를 제거했다. 대신 `git status` 확인, 관련 파일만 변경, 채널 절차와
  현재 요청 범위에 따른 커밋·푸시·배포로 바꿨다.
- PII, 사진 권리, 실게시 게이트, 인증/CAPTCHA 우회 금지, 브랜드 제0원칙은 보존했다.
- 결과적으로 `AGENTS.md`와 `CLAUDE.md`에서 97줄을 제거하고 25줄로 대체했다.

### 검증·모델 기록

- `git diff --check` 통과. 변경 파일은 문서 2개뿐이었고 당시 별도 사용자 변경은 없었다.
- 수행·최종 판단: GPT-5.6 Sol medium, 실제 파일·git 이력 대조.
- Claude Fable 5 medium 독립 검증을 CLI로 요청했다. 첫 호출은 응답 본문이 반환되지 않았고,
  재호출에서는 조직이 Claude Code 구독 접근을 비활성화했다는 오류가 확인됐다. 따라서 검증
  근거로 사용하지 않았고 성공했다고 기록하지 않았다.
- 주요 자체 반박: 하네스를 과도하게 줄이면 개인정보·저작권·게시·인증 안전장치가 사라질 수 있다.
  최종 수용: 중복·시점성·도구별 설명만 제거하고 안전·브랜드 계약은 유지했다.

### 다음 세션 진입점

- 마케팅 원칙은 `CLAUDE.md`, 모델·도구 규칙은 `AGENTS.md`, 현재 채널 상태는 해당
...
- 다음 작업: 네이버 UI 작업은 운영자가 로그인한 브라우저에서 직접 확인할 수 있을 때만 재개한다.
  새 글을 만들 때는 카드 사실 검증을 재사용하되 초안 결과를 사실로 간주하지 않는다.
...
- 수행: GPT-5.6 Sol medium. 실제 파일,
  git 이력, `npm run activation:status` 출력으로 재검증했다.
---

## 2026-08-04 — foresttour.kr 트래픽·사진 표기·한국어 줄바꿈 (세션 인계)

### 요청과 반영 결과

- 분산돼 있던 `/admin/traffic`의 foresttour.kr 지표를 홈 유입부터 예약 완료까지 한 여정으로
  통합했다. 예약 완료 지표는 클릭하면 동일 기간의 예약 목록을 펼치며, 관리자에게 필요한 최소
  필드만 보여 주고 민감정보는 제외한다.
- 사진 위에 삽입되던 안내 문구와 크레딧 오버레이를 제거했다. 후속 요청에 따라 공개 화면에는
  `출처 · 촬영자/제공처`만 남기고 CC BY·CC0·Public Domain 같은 라이선스 문자열은 숨겼다.
  라이선스 원본은 삭제하지 않고 권리·게시 게이트에 필요한 내부 메타데이터로 유지했다.
- 홈 대표 제목의 잘못된 분절을 교정했다. 최초 수정안 `당신이 아는 일본의` / `다음 장`은
  문장 의미가 덜 자연스럽다는 사용자 지적을 수용해 `당신이 아는` / `일본의 다음 장`으로
  확정했다. 페이지 전반에는 한국어 단어 중간 분리를 막고 제목과 문단의 줄을 자연스럽게
  배분하는 규칙을 적용했다.
- 예약 앱 관련 커밋은 `7ef04af`, `a274892`, `670a1e2`, `2bb1c15`, `8e83480`이며 모두
  `/home/kkokk/AX/reservation`의 원격 `main`에 반영됐다.

### 검증과 남은 확인

- 예약 앱 최종 검증: 테스트 88/88, TypeScript, 변경 파일 ESLint, Next.js 프로덕션 빌드 통과.
- 390px 브라우저에서 트래픽 화면의 가로 넘침 없음과 예약 완료 6건 목록 노출을 확인했다.
- 마지막 한국어 줄바꿈 변경은 공유 브라우저 프로필 잠금과 로컬 주소 입력 시 한국어 IME 간섭으로
  자동 브라우저 재확인을 완료하지 못했다. 다른 세션을 강제 종료하지 않았고 코드·테스트·빌드
  결과만 확정 근거로 사용했다. 다음 화면 작업 때 실제 모바일 폭에서 재확인한다.
- 예약 작업트리의 대규모 CRLF 오염은 의미 있는 변경과 분리해 정리했고, 고객명단·환경파일·업로드
  자료는 스테이징하지 않았다. 정리 및 기능 커밋은 이미 원격에 반영돼 있다.

### 판단·모델 기록

- 수행·최종 판단: GPT-5.6 Sol medium. 새 AGENTS 워커 규칙에 따라 다른 모델이나 CLI로
  재위임하지 않았으며 실제 파일·테스트·빌드·가능한 브라우저 화면으로 검증했다.
- 주요 반박 1: 식별자를 저장하지 않는 집계값을 고유 사용자 수처럼 합산할 수 없으므로 화면은
  사람 수가 아니라 단계별 활동 건수로 해석해야 한다.
- 주요 반박 2: 라이선스 데이터를 실제로 삭제하면 공개 권리 게이트가 깨질 수 있다. 사용자의
  의도는 화면 단순화로 판단해 공개 표기에서만 제거하고 내부 권리 메타데이터는 유지했다.
- 주요 반박 3: 첫 제목 수정은 문법상 가능해도 읽는 리듬이 좋지 않았다. 사용자 반박을 수용해
  `일본의 다음 장`을 한 의미 단위로 유지했다.

### 다음 세션 진입점

- 화면·트래픽 계약은 `HANDOFF-BLOG.md` §9를 먼저 읽는다.
- 예약 앱은 현재 `main...origin/main` 동기화 상태다. 마케팅 저장소의 기존 `package-lock.json`
  변경은 이 세션 소유가 아니므로 수정하거나 커밋하지 않았다.

---

## 2026-08-04 — 전용 자동화 런너 사용 종료

- 전용 자동화 런너와 그 브라우저 조작 경로는 폐기했다. 이후 작업은 운영자가 직접 확인한
  브라우저 화면, 공개 URL, 저장소 파일과 테스트만 근거로 삼는다.
- `AGENTS.md`, 채널별 인계서, 운영 문서에서 실행 방법·경로·환경변수 지시를 제거했다.
- 이 LOG의 과거 세션 블록에 남은 런너 명칭은 당시 작업을 설명하는 역사 기록이며, 새 작업의
  실행 지침이나 복구 대상이 아니다.
