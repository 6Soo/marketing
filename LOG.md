...
## 2026-07-26 세션 핸드오프 — 인스타 포맷 재설계·게시 안전성

### 구현

- `히다 001` 디자인을 고정 기준본에서 MVP 기록으로 낮추고 `cardnews/tools/template.mjs`를
  `evidence-v2`로 개편:
  - 표지 도장·전면 스크림·반복 대형 명조 제거
  - 현지 사진과 하단 편집 메모 영역 분리
  - 요약 카드는 저장 가능한 정보 카드로 유지
- 최신 2026 자료와 대체 수단을 `strategy/인스타그램-포맷-실험-0726.md`에 정리:
  캐러셀·Trial Reel·원본 1장·스토리 4셀 비교, 카드뉴스 폐기 조건, Collab·위치 유통·검색형 기록·
  현장 질문형 숏폼·YouTube Shorts 대안을 포함.
- 실게시 직전 최근 Instagram 미디어 최대 100건의 캡션을 조회해 동일 본문이면 실패 폐쇄.
  조회 실패도 게시 차단.
- 로컬 직접 실행도 첫 공개 자산이 HTTP 성공 + `image/jpeg`인지 확인한 뒤에만 API 호출.
- `refresh-token`이 새 토큰을 로그에 출력하던 경로 제거. `--output-file=<새 비공개 파일>`만 허용하고
  기존 파일 덮어쓰기를 차단.
- 위 동작의 가짜 API 자동 테스트 3건 추가.

### 검증·운영 상태

- 중복 차단·고유 캡션 진행·토큰 출력 차단 테스트 3건 통과.
- 새 디자인 대표 3장(표지·사진 내지·요약)을 1080×1350으로 렌더해 시각 확인.
- Windows Edge가 전체 일괄 렌더 중 캡처 뒤 종료를 지연하는 현상이 있어 전체 산출 PNG는 커밋하지 않음.
  생성에 사용한 Node/Edge 프로세스는 종료.
- 원격 브랜치 직접 조회 결과 `Master`만 존재하고 `instagram-assets`는 없음. 따라서 공개 자산 브랜치를
  사용하는 GitHub Actions 실게시 성공 이력은 아직 없음.
- 워크플로 YAML의 예약 트리거는 비활성이고 `publish_live=false`가 기본. GitHub Actions UI의
  워크플로 상태와 Secrets 존재 여부는 이 환경에서 확인하지 못했으므로 자동 게시 가능하다고 단정 금지.
- 산리쿠는 `photoStatus: placeholder`이며 현재 Pexels 대역 사진이라 계속 실게시 차단.
- 이번 세션 실제 Instagram 게시 없음.

### 다음 우선순위

1. 산리쿠 실제 현지 원본과 권리 문서 수급 후 사진-장소 일치 검증.
2. GitHub Actions UI에서 workflow 활성 상태, `IG_USER_ID`·`IG_ACCESS_TOKEN` Secrets, 토큰 만료일 확인.
