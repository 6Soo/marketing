...
## 2026-07-30 — Instagram 72h 체크포인트 장애 복구

- 실패 실행 `30485400588`의 로그를 확인했다. FEED/CAROUSEL 미디어에 지원되지 않는
  `profile_visits,follows`를 요청해 Graph API code 100이 발생한 것이 1차 원인이었다.
- `tools/ig-insights.mjs`가 미디어 유형별 메트릭을 선택하도록 수정하고 회귀 테스트를 추가했다
  (`564796a`). 수집 성공 뒤에도 기록 단계가 건너뛰어지는 독립적인 Actions 조건식 오류를
  발견해, 실제 스냅샷 파일 존재 여부를 실행 단계에서 검증하도록 수정했다
  (`3bb6b17`, `68e7102`).
- `npm run test:instagram` **74/74 통과**.
- 사도 복구 실행 `30503086785`, 북알프스 복구 실행 `30503139708` 모두 전체 성공.
  자동 푸시된 Graph API·foresttour 기록을 다시 받아 로컬 보고서를 실행했고, 두 실험 모두
  72h `recorded` 및 missing source group 없음으로 확인했다.
- 수행 모델: **GPT-5.6 Sol**. 검증 모델: **별도 GPT-5.6 Sol medium 에이전트**.
  주요 반박: 코드·단위 테스트만으로 확정하지 말고 실제 Actions 성공과 영속 기록을 최종
  증거로 요구해야 한다. 최종 수용: 두 복구 실행, 자동 푸시 파일, 로컬 보고서까지 확인한 뒤 확정했다.
- 폴백 사유: 현재 런타임에서 Claude Fable 5/Opus 5를 호출할 수 없고 리포의 Gemini 브리지는
  `.env`가 없어 사용할 수 없었다. 호출하지 않은 모델은 검증 기록에 포함하지 않았다.

---

## 2026-07-31 — foresttour.kr 블로그 자동화·Instagram 의존 제거

### 목표 판정

- 블로그가 여행지의 발견→이해→신뢰→관련 기록→정확한 일정 연결을 자사 사이트 안에서
  완결하도록 만들었다.
- “Instagram 완전 대체”는 기각했다. 블로그는 고유 URL·검색·RSS·장문 맥락·일정 연결을
  대체하지만, 릴스 추천 도달·좋아요/댓글/DM의 사회적 역할은 같지 않다.

### 마케팅 리포 변경

- `blog/published/stories.json`: 4편 공개 원고 단일 원천.
- `tools/blog-draft.mjs`: 카드 시리즈와 사실 검증 문서에서 **로컬 초안 골격만** 생성.
- `tools/blog-content.mjs`: 엄격한 공개 검증과 명시적 `--publish` 승격. 자동 git 작업 없음.
- `tools/blog-content.test.mjs`: 공개본, Instagram 의존 문장, 사진 권리, 미완성 초안 차단 4건.
- `HANDOFF-BLOG.md`와 `strategy/foresttour-블로그-자동화-0731.md`: 운영·역할 경계·완료 기준.
- 기존 Instagram 자동화는 그대로 보존했다.

### 앱 리포 변경

- 홈, `/stories`, `/stories/<slug>`가 동일 원고를 읽는다.
- 원격 main JSON을 1시간 재검증하고, HTTPS·5초·1MB·스키마 계약 실패 시 번들 스냅샷으로 폴백.
- Blog/BlogPosting JSON-LD, 공개/수정일, OG, RSS, sitemap, robots를 추가.
- 목록→본문→관련 글→일정 선택을 글별 익명 집계하고 Google·네이버·다음·Bing 자연검색을
  Instagram과 분리했다.
- 새 slug의 계측 사건은 공개 원고에 실제 글이 있을 때만 서버가 허용해 임의 이벤트 cardinality를 막는다.

### 검증

- 마케팅 `blog:validate` 통과, `test:blog` **4/4**, 기존 `test:instagram` **74/74**.
- 앱 전체 테스트 **86/86**, TypeScript, 변경 파일 ESLint, Next.js 16.2.9 프로덕션 빌드 통과.
  전체 ESLint는 이번 변경과 무관한 기존 오류 5건 때문에 실패하며 변경 파일은 0건이다.
- 로컬 프로덕션 브라우저: 목록·개별 글 렌더, 링크 이동, JSON-LD·날짜, 오류 오버레이·콘솔 오류 없음.
- 홈·목록·글 4편·OG 4장·홈 OG·RSS·sitemap·robots의 **14개 URL 계약 통과**.

### 모델 교차검증

- 수행·최종 판단: **GPT-5.6 Sol medium**.
- 1차 검증: **Claude Fable 5 medium**. 주요 반박은 “공개 원고 봇이 PII 예약 리포를 자동
  커밋하면 동기화 규칙과 목표 범위를 위반한다”, “블로그가 Instagram의 추천 도달과 사회적
  역할까지 대체한다고 쓰면 과장이다”였다. 둘 다 수용해 읽기 전용 원격 원고+정적 폴백과
  역할 경계로 교정했다.
- 저추론 구현 검토: **AGY Gemini 3.6 Flash high**는 구체 검토 대신 CLI 사용법만 반환해 폐기.
  지정 폴백 **AGY Claude Sonnet 4.6**의 런타임 스키마 불일치·XML escaping·JSON-LD 안전 처리·
  서버/클라이언트 경계·PII 없는 계측 반박을 수용했다.
- 폴백 사유: Flash 응답이 작업 결과가 아니어서 AGENTS.md 순서에 따라 Sonnet으로 넘겼다.
- Fable 최종 반박 대조:
  - 일정 시점 부패 P0는 앱이 매 렌더에서 공개 투어 API의 URL·날짜·국가·제목을 다시 검증하고
    없으면 CTA를 숨기는 실제 코드·테스트로 기각.
  - 사진 권리·XSS 우려는 verified/placeholder 게이트, 화면 크레딧, 본문을 React 텍스트로만
    렌더하고 JSON-LD의 `<`를 escape하는 실제 코드로 기각.
  - 폴백의 긴급 삭제 되살림, 폴백 무관측, 전체 referrer 전달 우려는 수용. 앱 번들 동시 삭제·
    재배포 절차, Vercel 경고 로그, referrer URL 미전송으로 교정.
  - 검색 색인 전에는 발견 독립이 실증되지 않았다는 반박을 수용. 4주 실제 색인·검색 유입을
    전략 성공 기준으로 유지한다.
...
## 2026-07-31 — 네이버 검색 유입 계약·비간섭 운영 보강

- 사용자가 작업 중 마우스 제어 방해를 지적했다. 이후 보이는 WSL 브라우저에는 입력을 전혀 보내지
  않았고, 이전 제목이 들어간 미저장 SmartEditor 초안도 그대로 두었다. 앞으로 `open`·`stage`·
  `verify`는 실행 직전 사용자 허락을 받는다.
- 제한적인 네이버 비로그인 검색 결과 제목 대조에서 `사도섬 여행`은 여수 사도·신안 12사도와
  의도가 충돌했다. 로컬 정본 제목과 첫 문단을 `일본 니가타 사도섬 여행` 중심으로 바꿨다.
  네이버 데이터랩 상대 검색량은 자격정보가 없어 확인하지 못했고 추정하지 않았다.
- `naver-blog/growth-plan.json`에 사도 기둥 글과 가는 법·섬 내 교통·2박 3일 지원 질문을 분리했다.
  주 2편 이하·최소 3일 간격, 지원 글당 신규 공식 출처 2개·독립 검증 사진 2장·본문 중복률 25%
  이하를 강제한다.
- `tools/naver-blog-growth.mjs`와 `naver-blog/observations.json`을 추가했다. 공개 URL이 검증된 글만
  D+3·D+7·D+28의 목표 검색어 발견·순위 구간·조회·검색 유입·foresttour 클릭 집계를 기록한다.
  개인별 검색어·계정 등 허용되지 않은 필드는 거부한다.
- 별도 Flash 검토는 P0/P1 없음으로 판정했으나 GPT-5.6 Sol 재반박에서 수동 slug 삽입과 조기
  체크포인트 기록 우회를 발견했다. 전체 `published.json` 계약과 실제 `verifiedAt` 이후 3·7·28일
  경과를 추가 검증해 수용·수정했다. 보이는 브라우저 명령도 `--user-approved` 없이는 거부한다.
- 대표 검색어뿐 아니라 다른 글의 보조 검색어까지 정확히 겹치면 잠식으로 차단한다. 기둥 글의
  보조 검색어에서 지원 글 대표 검색어 2개를 분리했다.
- 검색 유입 회귀 테스트를 포함한 `test:naver-blog` 18/18과 성장 계획 검증·보고가 통과했다.
  현재 보고는 사도 기둥 글 `staged · 공개 미확인`, 상대 검색량 미확인, 지원 글 3/3 추가 조사
  필요로 정확히 표시한다. 실제 공개·검색 유입 성과는 아직 없다.
- 수행·최종 판단: **GPT-5.6 Sol medium**. 읽기 전용 검증: **AGY Gemini 3.6 Flash high**.
  Flash는 P0/P1 없음으로 판정했지만, Sol이 공개 기록 위조·조기 체크포인트·보조 검색어 잠식
  우회를 추가로 찾아 반박하고 수정했다. **Claude Fable 5와 폴백 Opus 5는 현재 AGY 모델 목록과
  런타임에 없어 호출하지 못했다**. 제공되는 Opus 4.6을 Opus 5인 것처럼 대신 기록하지 않았다.

---

## 2026-07-31 — 사도 가는 법 검색 지원 글 독립 초안

- 보이는 브라우저와 SmartEditor에는 입력을 보내지 않았다. WSL 백그라운드의 공식 원문 조회,
  Commons API, 로컬 파일·테스트만 사용했다.
- 첫 지원 질문 `일본 사도섬 가는 법`에 대해 니가타역·공항→니가타항, 카페리·제트포일 비교,
  온라인 예약·디지털 승선권, 당일 승선, 수하물, 지연·결항 대응을 4,935자로 작성했다.
- 사도기센·니가타공항 공식 출처 7개를 연결했다. 기존 기둥 글 출처와 URL이 다른 신규 원문은
  6개이며, 시각·운임은 확인일과 변경 가능성 고지를 함께 둔다.
- Wikimedia Commons에서 기존 기둥 글과 다른 료쓰항 터미널 외관과 실제 발권·개찰 공간을
  가져왔다. 두 사진은 CC BY 4.0/2.0, 3840×2880, 고정 리비전·SHA-256을 보존했다. CC BY-SA와
  장변 2700px 미만 사진은 게이트에서 거부한다.
- `tools/naver-blog-support.mjs`가 공식 도메인, 출처 참조, 사진 바이트·메타데이터·리비전·권리,
  기둥/지원 글 사진 중복, 검색 의도, 내부 링크, 상품 미연결, 5어절 본문 중복률을 검증한다.
- 실측 본문 중복률은 1.3%로 25% 상한보다 낮다. `sado-access`는 `draft-ready`지만 기둥 글 공개
  URL이 `published.json`에 없으므로 로컬 미리보기만 만들고 발행 준비는 fail-closed로 막았다.
- 실제 검색 노출·유입·클릭은 아직 0이 아니라 **미관측**이다. 나머지 지원 글 2편은
  `research-required`로 유지해 데이터 없는 기계적 양산을 하지 않았다.
- 네이버 테스트 25/25, 자사 블로그 4/4, Instagram 74/74와 모든 성장·지원 글 검증이 통과했다.
- 수행·최종 판단: **GPT-5.6 Sol medium**. 읽기 전용 검증: **AGY Gemini 3.6 Flash high**.
  Flash는 6개 P0/P1 항목 모두 이상 없음으로 판정했다. 다만 검토문에 공항 미니라이너 운임을
  `1,300엔`이라고 잘못 적었고 이는 실제 공식 페이지의 `1,000엔` 및 초안 내용과 다르므로 기각했다.
  초안은 변동 가능성이 큰 운임 액수를 쓰지 않고 `현금 결제`와 공식 링크만 유지한다. Fable 5와
  Opus 5는 현재 런타임에 없어 호출하지 않았으며 다른 모델을 가장하지 않았다.

---

## 2026-07-31 — 검색 지원 글을 공통 발행 검증 경로에 연결

- 보이는 브라우저와 SmartEditor에는 입력을 보내지 않았다. 파일·테스트·읽기 전용 모델 검토만
  수행했다.
- `naver-blog:support:prepare`를 추가했다. `published.json`에 기둥 글의 검증된 공개 URL이 없으면
  package·post·manifest를 만들기 전에 실패하고, 공개 기록이 있을 때만 결정적인 지원 글 패키지와
  로컬 사진 2장을 `_stage`에 쓴다.
- 지원 글 패키지를 공통 `naver-blog:validate`, `stage`, `verify`가 읽도록 연결했다. 초안·공식 출처·
  사진·기둥 URL 중 하나라도 현재 정본과 달라지면 재생성 비교가 실패한다.
- 공통 stage 진입 전 manifest의 이미지 수·순서·안전한 상대 경로·SHA-256·바이트 수·캡션·출처를
  패키지 및 실제 파일과 다시 대조한다. 경로 이탈과 파일 변조를 브라우저 입력 전에 차단한다.
- 1차 독립 검토는 P0/P1/P2 없음으로 판정했지만, GPT-5.6 Sol 재반박에서 지원 글 공개 뒤
  `published.json`과 growth-plan `published` 상태를 지원 초안 검증이 동시에 수용하지 못하는
  수명주기 충돌을 새로 발견했다. 공개 기록 유무로 기대 상태를 계산하도록 고치고 회귀 테스트를
  추가했다.
- 지원 글 이미지 파일명은 원본의 `01-`, `02-` 순번을 그대로 유지해 중복 접두사를 제거했다.
- 로컬 단위·통합 테스트는 수정 전 `test:naver-blog` **29/29**를 통과했다. 실제 현재 상태의
  `naver-blog:support:prepare -- --slug=sado-access`는 기둥 공개 URL 부재로 실패해야 하며,
  exit 1과 package 미생성을 별도 명령으로 재확인했다. 수명주기 수정 뒤 전체 회귀 결과와 최종
  모델 판정은 다시 기록한다.
- 최종 회귀는 네이버 **30/30**, 자사 블로그 공개 게이트와 테스트 **4/4**, Instagram **74/74**를
  통과했다. 지원 글 계약·성장 계획도 현재 empty registry에서 통과하며 성과 관측은 0이 아니라
  **미관측**으로 유지된다.
- 수행·최종 판정: **GPT-5.6 Sol medium**. 읽기 전용 검증: **AGY Gemini 3.6 Flash high**.
  1차 Flash는 P0/P1/P2 없음으로 판정했지만 Sol이 위 수명주기 충돌을 찾아 반박·수정했다. 수정 뒤
  2차 Flash는 공개 전후 상태, stale digest, 동적 import, manifest 검증에서 P0/P1 없음으로 판정했고,
  Sol이 실제 파일과 30개 테스트로 다시 확인해 수용했다. Flash 1차 보고의 잘못된 파일 라인과
  실행하지 않은 `test:band` 언급은 증거로 쓰지 않았다.
- 폴백 사유: `agy models` 실측 목록에 **Claude Fable 5와 Opus 5가 없었다**. 제공되는 Opus 4.6을
  Opus 5로 가장하지 않았고, 호출 가능한 지정 저추론 검증 모델 Flash를 사용한 뒤 Sol이 최종
  재반박했다.

---

## 2026-08-01 — 네이버 블로그 공개·사진 출처 최소화·북알프스 현장형 문체 보강

- 브라우저 조작은 WSL의 `orca-ide` computer-use CLI로만 수행했다. 작업 전 `status`와
  `computer capabilities`를 확인했고, 변경 뒤 새 접근성 트리와 공개 URL로 다시 검증했다.
- 블로그 이름은 `숲길따라 감성여행`이다. 사도 기둥 글을 공개했고, 사진마다 반복되던 번호·출처·
  원문 URL을 모두 제거해 본문 마지막 한 줄의 최소 권리표시만 남겼다.
  - 사도: https://blog.naver.com/kkokko_hero/224364700027
  - 최소 표기: `사진: ccfarmer(CC BY 3.0), amaknow·rhodnite(CC BY 2.0), JH0WJF(PD)`
- 북알프스 글을 사진 9장 중심으로 공개한 뒤, 사용자 피드백에 따라 편집 화면에서 사진은 보존하고
  세 구간의 문단만 현장 동행기처럼 다시 썼다. 첫 문장은 `무로도에 닿자 자동문 너머에서 찬 바람이
  훅 들어온다.`이고, 셔터·바람·강물·앨범 같은 감각어로 사진 사이의 장면 흐름을 만들었다.
  - 제목: `일본 북알프스 여행 | 무로도·구로베댐·가미코지`
  - 공개 URL: https://blog.naver.com/kkokko_hero/224364780223
  - logNo: `224364780223`
  - 사진 9장, 주제 `세계여행`, 카테고리 `낙서장`
  - 전체공개·댓글·공감·검색·블로그/카페 공유·외부 공유가 모두 켜진 것을 발행 직전 화면으로 확인
  - 사진별 출처·원문 블록은 없고 마지막에 권리표시 한 줄만 유지
  - 상품 연결: https://reserve.foresttour.kr/tour/fNod?from=naver-blog
  - 실제 공개 태그에는 입력 중 합쳐진 `다테야마알펜루트미쿠리가이케`가 하나 더 남아 있다.
- 실제 방문·직접 촬영 사실을 확인할 증거는 없으므로 이를 명시적으로 주장하지 않았다. 대신 독자가
  장면 안에 서 있는 듯 읽히는 현재형·감각 중심 문체를 사용했고, 사진 권리표시가 외부 사진임을
  투명하게 남긴다.
- `tools/naver-blog-content.mjs`는 앞으로도 사진별 출처 블록 대신 한 줄 사진 권리표시를 생성한다.
  Public Domain은 `PD`로 축약한다. 사도 패키지를 새 계약으로 재생성했고 공개 기록을
  `naver-blog/published.json`에 저장했다. 성장 계획의 사도 기둥 글은 `published`로 갱신했다.
- 공개 접근성 트리에서 북알프스 새 문장, 본문 사진 링크 9개, 상품 URL, 최소 사진 표기를 확인했고
  `사진 출처:`, `사진 원문:`, Commons 원문 URL 반복이 없음을 재검증했다.
- 회귀 결과: `test:naver-blog` **30/30**, 성장 계획 검증, `git diff --check` 통과.
- 수행 모델: **GPT-5.6 Sol medium**. 검증 모델: 지정된 Fable 5/Opus 5를 호출할 수 없어
  **GPT-5.6 Sol medium 자체 반박 검증**으로 폴백했다. 저추론 초안은 `GEMINI_API_KEY`가 없어
  AGY Gemini 3.6 Flash bridge를 실행하지 못했다.
- 주요 반박: 현장형 문체가 확인되지 않은 실제 방문·직접 촬영 주장으로 넘어갈 위험이 있다.
  최종 수용: 장면 순서·감각·현재형은 강화하되 `내가 직접 방문했다`, `직접 찍었다` 같은 사실 주장은
  넣지 않고, 최소 사진 권리표시로 외부 사진 사용을 공개한다.

### 세션 핸드오프

- 완료: 사도·북알프스 공개 URL 검증, 북알프스 현장형 문체 재발행, 두 글의 사진 출처 최소화.
- 런타임: Edge PID `18104`, window ID `329022`; 공개 북알프스 글이 활성 페이지다.
- 미결: 북알프스 합성 태그 1개 정리 여부와 D+3·D+7·D+28 성과 관측. 태그 하나 때문에 본문을
  다시 편집하는 위험은 감수하지 않았다.
- 함정: 보이는 글 편집 시 해당 문단이 화면 안에 있어야 접근성 클릭이 정확하다. 화면 밖 문단을
  클릭하면 직전 문단에 붙을 수 있으므로 좌표 스크롤 뒤 새 트리로 확인한다.
- 부트스트랩: `orca-ide status` → `orca-ide computer capabilities` → Edge window 재조회 →
  공개 URL의 새 접근성 트리 확인 순서로 시작한다.

---

## 2026-08-01 — 북알프스 사진·글 교차 편집 완료 / 세토우치 신규 글 발행 대기

- 북알프스 공개 글을 사진 9장과 여행 블로그 문단 9개가 정확히 번갈아 나오는 구조로 다시
  편집했다. 공개 접근성 트리에서 `사진 → 글` 쌍 9개와 기존 여행 메모·상품 연결·최소 권리표시를
  재확인했다. 실제 방문·직접 촬영 사실은 주장하지 않고 장면과 감각을 현재형으로 설명했다.
  - 공개 URL: https://blog.naver.com/kkokko_hero/224364780223
- 예약 리포의 공개 상품 API에서 2026년 9~12월 상품 34개를 대조하고, 사진 중심의 독립 여행지
  소개와 섬별 배편·시간 지정 미술관 예약의 실제 준비 난점을 함께 보여줄 수 있는
  `세토우치 예술섬 — 나오시마·테시마·이누지마·구라시키`를 다음 지역으로 골랐다.
  - 공개 상품: https://reserve.foresttour.kr/tour/fNRs?from=naver-blog
  - 2026-09-18 출발, 5박, 오카야마 왕복 대한항공, 6명 이상 출발
- 나오시마·테시마·이누지마·세토 내해·구라시키 사진 9장을 Commons의 CC BY 허용 원본으로
  준비하고 육안 확인했다. 원고는 `사진 + 짧은 현장형 문단` 9쌍, 여행 메모, 낮은 압력의 상품
  연결, 본문 마지막 사진 권리표시 한 줄로 구성했다. 사진별 출처 블록은 만들지 않았다.
  - 준비물: `_stage/naver-blog/setouchi/`
  - Windows 업로드 폴더: `C:\\Users\\kkokk\\Desktop\\setouchi-blog`
- 공식 원문으로 2025-05-31 나오시마 신미술관 개관, 지중·테시마 미술관 등의 날짜·시간 지정
  입장권, 2026년 미야노우라–테시마–이누지마 고속선 하루 3편과 휴관일 연동 운항, 구라시키
  미관지구의 흰 회벽·나마코벽·버드나무 경관을 확인했다.
- 브라우저는 WSL `orca-ide` computer-use CLI만 사용했다. Edge의 새 글 편집기까지 열었지만
  `AhnLab Safe Transaction - 인터넷 연결 탐지` 창이 최상위에서 키보드 입력을 막고 있다.
  Orca로 닫기만 여러 방식으로 시도했으나 창이 유지됐고, `허용/차단`은 보안 결정을 대신하지
  않기 위해 선택하지 않았다. 운영자가 이 창의 우측 상단 X만 닫으면 제목 입력부터 재개한다.
- 수행 모델: **GPT-5.6 Sol medium**. 검증 모델: Fable 5/Opus 5와 Gemini bridge를 현재 런타임에서
  호출하지 못해 **GPT-5.6 Sol medium 자체 반박 검증**으로 폴백했다. 주요 반박은 현장형 말투가
  확인되지 않은 방문·촬영 주장으로 넘어갈 위험이며, 감각 묘사만 강화하고 1인칭 방문·촬영
  단정은 넣지 않는 것으로 최종 수용했다.

### 세션 핸드오프

- 완료: 북알프스 사진·글 9쌍 공개 검증, 9~12월 공개 상품 대조, 세토우치 사진·원고·업로드
  manifest·메타데이터 준비.
- 미결: AhnLab 탐지창 X 닫기 → 세토우치 새 글에 제목·사진 9장·문단 9개 입력 → 공개 설정 확인
  → 발행 → 공개 URL 접근성 트리 검증 → `published.json` 기록 → 회귀 테스트.
- 런타임: Edge PID `18104`, window ID `329022`, 새 글 편집기
  `https://blog.naver.com/kkokko_hero/postwrite?categoryNo=1`가 선택된 상태다.
- 보안창: StSess PID `25980`, window ID `19598600`. `허용/차단`을 대신 선택하지 말고 운영자가 X로
  닫은 뒤 Edge의 새 접근성 트리부터 다시 읽는다.

---

## 2026-08-04 — 세토우치·야쿠시마 발행 준비 / 네이버 재인증 차단 / 세션 전면 인계

### 사용자 확정 요구

- 블로그 이름은 `숲길따라 감성여행`입니다.
- 글의 주인공은 상품이 아니라 여행지입니다. 사진이 글보다 많고, 개인 여행 블로그처럼 부드러운
  말투와 현장감 있는 현재형을 씁니다.
- 편집 형식은 사진 1장과 짧은 글 1개를 번갈아 두는 구조이며 현재 기준은 9쌍입니다.
- 독자가 직접 갈 수 있을 만큼 여행 정보를 충분히 주되, 섬별 배편·예약·날씨·긴 이동처럼 실제
  조율 비용이 자연스럽게 보이게 하고 정확히 일치하는 숲길여행 일정으로 낮은 압력으로 연결합니다.
- 정보 일부를 일부러 숨기거나 `혼자 못 간다`고 압박하지 않습니다.
- 외부 사진의 출처·라이선스는 사진마다 반복하지 않고 본문 끝 최소 권리표시 한 줄만 유지합니다.
- 직접 방문·직접 촬영 증거가 없으므로 그런 사실을 1인칭으로 단정하지 않습니다. 감각·장면 순서·
  현재형으로 독자가 현장에 있는 듯한 문체만 강화합니다.
- 사용자는 컴퓨터를 직접 쓸 수 없는 상황이라고 밝혔고, 브라우저 전체 조작과 AhnLab 탐지창의
  `허용` 선택을 명시적으로 승인했습니다. 이전 블록의 `허용/차단 미승인` 기록은 현재 승인 상태로
  정정합니다.
...
- 북알프스 공개 접근성 트리에서 사진 9장과 문단 9개의 정확한 교차 구조, 상품 링크, 최소 사진
  권리표시를 확인했습니다.
- `naver-blog/published.json`에는 사도와 북알프스만 기록했습니다. 사도 기둥 글은
  `naver-blog/growth-plan.json`에서도 `published`로 갱신했습니다.
- 사진별 출처 블록을 없애고 최소 권리표시를 생성하도록 `tools/naver-blog-content.mjs`와 회귀
  테스트를 갱신했습니다.
...
- 세토우치
  ...
  - 2026-09-18 출발, Commons 사진 9장, 사진·글 9쌍
  - 로컬: `_stage/naver-blog/setouchi/`
  - Windows 업로드 사본: `C:\Users\kkokk\Desktop\setouchi-blog`
  - 추적 원고·원천·해시: `naver-blog/drafts/2026-08-04-setouchi.md`
- 야쿠시마
  - 제목: `야쿠시마 여행 | 시라타니 원시숲·조몬스기·폭포`
  - 상품: https://reserve.foresttour.kr/tour/fNhN?from=naver-blog
  - 2026-11-27 출발, Commons 사진 9장, 사진·글 9쌍
  - 로컬: `_stage/naver-blog/yakushima/`
  - Windows 업로드 사본: `C:\Users\kkokk\Desktop\yakushima-blog`
  - 추적 원고·원천·해시: `naver-blog/drafts/2026-08-04-yakushima.md`
- 두 지역은 **미발행**입니다. 공개 URL이 없고 `published.json`에도 기록하지 않았습니다.
- `_stage/`는 Git 제외 대상이므로 복사용 원고 전문, 공식 사실 원문, 사진별 Commons URL과
  SHA-256을 추적 문서에 별도로 보존했습니다. 이미지 바이트는 WSL과 Windows 두 위치에 존재함을
  2026-08-04에 재확인했습니다.

### 인증·브라우저 감사

- 모든 조작은 WSL에서 실행한 Orca computer-use CLI로 수행했습니다.
- AhnLab Safe Transaction의 인터넷 연결 탐지 창은 관리자 보호 무결성 경계에 있었습니다.
  Orca 클릭·키 입력, UIA, MSAA, PowerShell 입력, 화상 키보드까지 시도했지만 사용자 승인 뒤에도
  `허용` 버튼을 작동시킬 수 없었습니다. 보안창이나 인증을 우회하지 않았습니다.
- 정상 Edge를 종료하고 프로필을 임시 복제해 remote debugging/CDP 연결을 시험했습니다. Windows
  loopback CDP를 WSL에서 보기 위한 임시 Node relay와 PowerShell tunnel도 만들었지만,
  네이버 `NID_AUT`·`NID_SES`가 Cookie DB에 영속되지 않아 복제 프로필은 로그인 화면으로 갔습니다.
- QR 로그인을 여러 번 표시했으나 확인 전에 모두 만료됐습니다. 만료 코드는 재사용하지 않습니다.
- 임시 프로필과 QR 이미지는 `gio trash`로 휴지통에 보냈고, 임시 relay/tunnel 스크립트는
  `apply_patch`로 삭제했습니다. 원본 Edge 프로필은 덮어쓰지 않았습니다.
- 원래 Edge는 `--restore-last-session`으로 다시 열었고 Gmail과 기존 7개 페이지가 복원된 것을
  확인했습니다. 다만 강제 종료로 세션 전용 네이버 쿠키가 사라졌을 가능성이 커, 다음 세션은
  네이버 로그아웃 상태를 기본 가정으로 둡니다.
- 과거 Edge/StSess PID와 window ID는 재사용하지 않습니다. 항상 `status`·`computer capabilities`와
  새 접근성 트리로 다시 찾습니다.
- 채팅에 전달된 인증 문자열은 민감정보 가능성 때문에 파일·로그·커밋에 복사하지 않았습니다.

### 다음 세션 부트스트랩

1. `git pull --ff-only` 후 `HANDOFF-NAVER-BLOG.md`와 이 블록을 읽습니다.
2. `orca-ide status` → `orca-ide computer capabilities` → 현재 Edge/AhnLab 창 재조회 순서로
   시작합니다.
3. 네이버 로그인 여부를 확인하고, 로그인·2차 인증·CAPTCHA는 우회하지 않습니다.
4. 인증되면 세토우치 발행·공개 URL 검증·`published.json` 기록을 끝냅니다.
5. 곧바로 야쿠시마를 같은 방식으로 발행·검증·기록합니다.
6. `npm run test:naver-blog`, `npm run naver-blog:growth:validate`, `git diff --check`를 실행합니다.
7. 사도·북알프스의 D+3·D+7·D+28 검색 관측과 북알프스 합성 태그 정리는 별도 미결입니다.

### 모델·변경 범위 감사

- 최종 회귀: `npm run test:naver-blog` **30/30**, `naver-blog:growth:validate`,
  `naver-blog:support:validate`, JSON 파싱, 대기 사진 18장의 문서 SHA-256 대조,
  `git diff --check`가 모두 통과했습니다.
- 수행 모델: **GPT-5.6 Sol medium**
- 검증 모델: **GPT-5.6 Sol medium 자체 반박 + 실제 파일·공개 URL·접근성 트리·테스트**
- 주요 반박: 현장형 문체를 방문·촬영 사실로 오인시키면 안 되고, 승인된 AhnLab 선택 실패를
  미승인으로 기록하면 안 되며, 공개 URL 없는 두 대기 원고를 발행 완료로 기록하면 안 됩니다.
- 최종 수용: 감각 묘사와 최소 권리표시는 유지하고, 대기 원고·공식 원문·사진 해시를 Git 추적
  문서에 보존하며, 공개 상태는 실제 네이버 URL로만 판정합니다.
- 폴백 사유: 현재 워커 지침이 다른 모델·CLI로 재위임하지 말라고 명시해 Fable 5, Opus 5,
  AGY Gemini를 새로 호출하지 않았습니다. 호출하지 않은 모델은 검증 모델로 기록하지 않았습니다.
- `package-lock.json`의 플랫폼별 `libc` 메타데이터 삭제는 이 네이버 블로그 세션과 무관한 기존
  워킹트리 변경이므로 커밋 범위에서 제외하고 보존합니다.

### 세션 핸드오프

- 완료: 공개 2편 상태 기록, 사진 출처 최소화 구현·테스트, 세토우치·야쿠시마 원고·사진 18장 준비,
  두 대기본의 추적 문서화, 인증 실패·브라우저 복구·임시 자산 정리 기록.
- 최우선 미결: 네이버 재인증 → 세토우치 발행·검증 → 야쿠시마 발행·검증.
- 공개되지 않은 것을 공개됐다고 쓰거나, 만료된 QR·과거 PID·과거 window ID를 재사용하지 않습니다.
