...
작성 경로로 쓰지 않습니다. 따라서 종료된 API를 가장하지 않고, WSL에서 실행한 Orca
computer-use CLI로 로그인된 보이는 Windows Edge의 SmartEditor를 조작합니다. 사용자 본인 확인이
필요한 로그인·2차 인증·CAPTCHA는 우회하지 않고, 공개는 화면 검수와 명시적 승인 범위 안에서만
수행합니다. 무인 대량 발행은 하지 않습니다.
...
- 브라우저 감사·조작은 WSL 안에서 실행한 Orca computer-use CLI로만 수행합니다.
- 매번 `orca-ide status`와 `orca-ide computer capabilities`를 먼저 확인하고, Windows Edge 창과
  접근성 트리를 새로 조회합니다. 아래 §7의 과거 PID·창 ID는 재사용하지 않습니다.
...
- 로그인·2차 인증·CAPTCHA는 우회하지 않습니다. 로그인 프로필의 쿠키·토큰·QR 이미지는 저장소에
  기록하지 않습니다.
- 사용자가 마우스를 쓰는 상황이 아니라고 명시했으므로 현재 발행 작업에서는 컴퓨터 조작을 진행할
  수 있습니다. 새 세션에서 상황이 달라졌다는 메시지가 오면 즉시 입력을 멈춥니다.
...
2. Orca computer-use로 새 SmartEditor 탭을 열고 제목을 입력합니다.
...
- Windows Edge의 AhnLab Safe Transaction 인터넷 연결 탐지 창이 최상위 입력을 막았습니다.
  사용자는 `허용` 선택까지 명시적으로 승인했지만, Orca·UIA·MSAA·PowerShell·화상 키보드 입력은
  관리자 보호 경계를 넘지 못해 버튼을 작동시키지 못했습니다. 보안창을 우회하지 않았습니다.
...
2. `orca-ide status` → `orca-ide computer capabilities` → 현재 Edge/AhnLab 창 재조회 순서로 시작합니다.
