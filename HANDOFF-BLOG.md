...
산리쿠는 2026-08-04에 다로 해안·오사와 교량·기타야마자키·조도가하마, 히다는 다카야마
산마치·시라카와고·게로 온천의 Wikimedia Commons CC BY 현지 사진으로 교체했습니다. 두 글 모두
`photoStatus: verified`입니다. 산리쿠 트레일 절은 정확한 장면의 사진을 확보하지 못해 대역 사진을
남기지 않고 이미지를 생략했습니다. 원본 리비전, 라이선스, SHA-256은
`cardnews/photos/{sanriku,hida}/blog-*.jpg.source.json`에 있습니다.

placeholder 교체 우선순위는 완료했습니다. 새 지역은 카드 시리즈의 사실 검증을 재사용할 수 있지만,
초안 생성 결과를 사실로 간주하지 않습니다. 외부 검색 콘솔 후속은 운영자가 로그인한 브라우저에서
직접 확인할 수 있을 때 재개합니다.

## 10. 2026-08-05 사진 권리표시 최신 계약

- `blog/published/stories.json`의 northern-alps 사진 감사 8건은 `kind: photo-audit`로 표시한다.
  예약 앱의 이용자용 하단 `SOURCES`에는 이 감사 레코드를 노출하지 않고 공식 글 출처만 남긴다.
- 실제 렌더되는 CC BY/Public domain 사진에는 이미지 옆에 저작자·원문 링크와 라이선스 링크를 한 줄로
  표시한다. Pexels·AI·권리표시가 없는 이미지에는 이 행을 만들지 않으며, 기존 안내·검증 메타데이터는
  유지한다.
- 이 블록이 2026-08-04의 `라이선스 문자열은 공개 화면에 표시하지 않는다` 문구를 사진 귀속에
  관해 대체한다. 사진을 수정한 경우에는 변경 사실을 별도로 표시해야 한다.
- `placeholder` 교체, `photoStatus`, 원문 리비전·라이선스·SHA-256 보존, 마케팅 안전 게이트는
  변경하지 않는다.
