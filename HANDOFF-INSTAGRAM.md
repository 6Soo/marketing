...
>
> 최종 갱신: 2026-07-31 · 이 문서의 모든 수치는 **실측**이다(파일 직접 읽기 / 도구 직접 실행 /
> `gh` API 조회). 확인하지 못한 것은 "확인 못함"으로 적었다. 추정치는 없다.
>
> **2026-07-31 채널 경계 갱신:** foresttour.kr 블로그가 Instagram 없이도 여행지의
> 발견→이해→신뢰→일정 연결을 수행하는 자사 채널로 추가됐다. 블로그 공개 원고와 배포·검색·
> 익명 계측 인계는 `HANDOFF-BLOG.md`가 정본이다. Instagram은 비팔로워 추천 도달과 사회적
> 반응 실험을 계속 담당하며, 블로그를 Instagram의 알고리즘 도달과 동일하다고 간주하지 않는다.
...
7. **블로그는 독립적으로도 완결돼야 한다.** Instagram 캡션·계정·게시물이 없어도 제목·본문·공식 출처·
   사진 고지·관련 기록·정확한 일정 연결을 이해할 수 있어야 한다. 정본은 `HANDOFF-BLOG.md`.
...
| `strategy/foresttour-블로그-자동화-0731.md` | 블로그의 Instagram 대체 범위·자동화·완료 기준 |
| `HANDOFF-BLOG.md` | 블로그 운영 명령·데이터 계약·배포·검색 등록 인계 |
